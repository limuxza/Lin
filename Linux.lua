...
function Linux:SafeCallback(Function, ...)
    ...
    if not Success then
        local errorText = tostring(ErrorMessage or "Unknown error occurred")
        self:Notify({
    ...
    local success, result = pcall(function()
        if listfiles and isfolder(Linux.ConfigFolder) then
            ...
            for _, file in pairs(files) do
                ...
                if configName and configName ~= "autoload" and configName ~= "privacy" then
...
function Linux.Create(config)
    ...
    CloseButton.MouseButton1Click:Connect(function()
        Linux:Notify({Title = "Closed UI", Content = "Press LeftAlt to reopen.", Duration = 5})
        LinuxUI:Destroy()
        ToggleUI:Destroy()
    end)
